...
class MeasurementActivity : AppCompatActivity() {
    companion object {
        const val SERIES_ID = "MEASURE_ID"
        const val SERIES_NAME = "MEASURE_NAME"
    }
    ...
    private fun renderData(measures: List<Measure>?) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dataReceiver = DataReceiver(seriesId) { renderData(measureDao?.getBySeries(seriesId)!!) }
        ...
        if (!App.isDeviceConnected()) {
            App.DIALOG_UTIL.onCreateNoDeviceForkDialog(this).show()
        } else {
            Snackbar.make(
